...
-(void)viewDidLoad{
    ...
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithImage:[UIImage imageNamed:@"back_X_icon"]
    ...
    self.searchBar = [[UISearchBar alloc]initWithFrame:CGRectMake(65, 0, self.view.frame.size.width - 80, 44.0f)];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    ...
    GYZCityGroup *group = [self.cityDatas objectAtIndex:section - ShowAll];
...
{
    if (section < ShowAll || self.isSearch) {
        return 0.0f;
    }
    return 23.5f;
}
...
//        NSLog(@"array %@",array);
//        NSError *err;
//        NSData *da = [NSJSONSerialization dataWithJSONObject:array options:0 error:&err];
//        if (!err) {
//            NSString *ss = [[NSString alloc] initWithData:da encoding:NSUTF8StringEncoding];
//            NSLog(@"jaslfkjasl;kdfj;lasjdf %@",ss);
//        }
